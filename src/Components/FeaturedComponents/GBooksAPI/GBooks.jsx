...
const GBooks = () => {
    ...
    const apiKey = 'AIzaSyDz_uBQ8PWed_gkoGzlQM6bXjbAmdLGN80';
...
//             try {
//                 const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=mahatmagandhi&key=${apiKey}`); // Default search for fiction books
...
//         if (!searchTerm) { // Display all books if search term is empty
//             return true;
//         }
...
//         const authors = book.volumeInfo?.authors?.map((author) => author.toLowerCase()) || [];

//         return title.includes(lowercasedSearchTerm) || authors.some((author) => author.includes(lowercasedSearchTerm));
//     });
...
//     const handleSearchReq = async (event) => {
...
//                     `https://www.googleapis.com/books/v1/volumes?q=` + search + `=free-ebooks&key=${apiKey}`
//                 );
...
//             try {
//                 const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=` + search + `=free-ebooks&key=AIzaSyDz_uBQ8PWed_gkoGzlQM6bXjbAmdLGN80`); // Replace with your API endpoint URL
//                 const data = await response.json();
//                 setBooksData(data.items || []); // Handle potential data structure variations
//             } catch (err) {
//                 setError(err.message);
//             } finally {
//                 setIsLoading(false);
